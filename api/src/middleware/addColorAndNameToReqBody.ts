...
const addColorAndNameToReqBody = async (
  ...
  try {
    ...
    if (!variation) {
            res.status(400).json(createErrorJson([{ type: 'addVariationImg', msg: 'variation_not_found' }]));
            return;
    ...
    if (!product) {
       res.status(400).json(createErrorJson([{ type: 'addVariationImg', msg: 'product_not_found' }]));
       return;
    }
